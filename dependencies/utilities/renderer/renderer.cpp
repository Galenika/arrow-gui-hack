...
void render::setup() {
	...
	interfaces::surface->set_font_glyph(render::fonts::esp_font, "Tahoma", 12, 600, 0, 0, font_flags::fontflag_dropshadow);
...
void render::draw_text_string_alternative(int x, int y, unsigned long font, std::string string, bool text_centered, color colour) {
	...
	interfaces::surface->draw_text_font(font);
	if (text_centered)
		interfaces::surface->draw_text_pos(x - width, y);
	else
		interfaces::surface->draw_text_pos(x, y);
